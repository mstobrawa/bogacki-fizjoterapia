...
export default async function AdminOfferPage() {
  ...
  const { data: services } = await supabase.from("services").select("*").order("position").order("created_at");
    ...
    <Section tone="surface">
      ...
      >
        <AdminPanel title="Dodaj usługę">
          <form action={createService} className="grid gap-4 rounded-[1.35rem] border border-[var(--color-border)] bg-white p-5">
            <div className="grid gap-3 lg:grid-cols-[1fr_1fr_8rem]">
              <input required name="title" placeholder="Nazwa usługi" className={inputClass} />
              <input required name="price" placeholder="Cena, np. 150-230 zł" className={inputClass} />
              <input name="position" type="number" placeholder="Kolejność" className={inputClass} />
            </div>
            <textarea required name="description" placeholder="Opis usługi" className={areaClass} />
          ...
          {services?.map((service) => (
            <EditableCard key={service.id}>
              <form action={updateService} className="grid gap-3">
                ...
                <input type="hidden" name="current_storage_path" value={service.storage_path ?? ""} />
                <div className="grid gap-3 lg:grid-cols-[1fr_1fr_8rem]">
                ...
                <AdminImageField name="image" label="Zdjęcie usługi" currentUrl={service.image_url} />
                <Button type="submit" variant="secondary" className="justify-self-start">
                  Zapisz
                </Button>
              </form>
              <DeleteForm action={deleteService} id={service.id} storagePath={service.storage_path ?? ""} />
            </EditableCard>
          ))}
        </div>
      </AdminChrome>
