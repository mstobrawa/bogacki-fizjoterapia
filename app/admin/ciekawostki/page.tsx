...
import { AdminChrome, AdminPanel, DeleteForm, areaClass, inputClass } from "../_components/AdminChrome";
      ...
      >
        <AdminPanel title="Dodaj wpis">
          <form action={createPost} className="grid gap-4 rounded-[1.35rem] border border-[var(--color-border)] bg-white p-5">
            ...
            <AdminImageField name="image" label="Zdjęcie wpisu" previewClassName="h-28 w-44 rounded-[1rem] border border-[var(--color-border)] object-cover" />
            <textarea required name="content" placeholder="Treść wpisu" className={`${areaClass} min-h-36`} />
        ...
        <div className="mt-6 grid gap-3">
          {posts?.map((post) => (
            <article key={post.id} className="rounded-[1.35rem] border border-[var(--color-border)] bg-white p-4 shadow-sm">
              <div className="grid gap-4 sm:grid-cols-[6.5rem_1fr] sm:items-start">
                <div className="h-24 overflow-hidden rounded-[1rem] border border-[var(--color-border)] bg-[var(--color-surface)]">
                  {post.image_url ? (
                ...
                <div className="min-w-0">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <h2 className="text-base font-semibold text-[var(--color-primary)]">{post.title}</h2>
                      <p className="mt-1 text-xs font-semibold uppercase tracking-[0.14em] text-[var(--color-text-muted)]">
                        {post.published ? "Opublikowany" : "Szkic"}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <DeleteForm action={deletePost} id={post.id} className="" />
                    </div>
                  </div>
                  ...
                  <details className="mt-4">
                    <summary className="inline-flex min-h-10 cursor-pointer list-none items-center rounded-full border border-[var(--color-primary)] bg-white px-4 text-sm font-semibold text-[var(--color-primary)] shadow-sm transition hover:bg-[var(--color-surface)]">
                      Edytuj wpis
                    </summary>
                    <div className="mt-4 rounded-[1.15rem] border border-[var(--color-border)] bg-[var(--color-bg)] p-4">
                      <form action={updatePost} className="grid gap-3">
                        ...
                        />
                        <textarea required name="content" defaultValue={post.content} className={`${areaClass} min-h-36`} />
                        <label className="flex items-center gap-3 text-sm text-[var(--color-primary)]">
                          <input name="published" type="checkbox" defaultChecked={post.published} /> Opublikowany
                        </label>
                        <Button type="submit" variant="secondary" className="min-h-11 justify-self-start px-5">
                          Zapisz
                        </Button>
                      </form>
                    </div>
