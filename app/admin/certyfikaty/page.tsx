...
import { AdminChrome, AdminPanel, DeleteForm, EditableCard, inputClass } from "../_components/AdminChrome";
...
export default async function AdminCertificatesPage() {
  ...
  const { data: certificates } = await supabase.from("certificates").select("*").order("position").order("created_at");
    ...
    <Section tone="surface">
      ...
      >
        <AdminPanel title="Dodaj certyfikat">
          <form action={createCertificate} className="grid gap-4 rounded-[1.35rem] border border-[var(--color-border)] bg-white p-5">
            <div className="grid gap-3 lg:grid-cols-[1fr_8rem]">
              <input name="title" placeholder="Tytuł opcjonalny" className={inputClass} />
              <input name="position" type="number" placeholder="Kolejność" className={inputClass} />
            </div>
            <AdminImageField name="image" label="Zdjęcie certyfikatu" required />
