...
export function Hero() {
  const newLocal =
    "absolute -bottom-1 left-0 h-1 w-full rounded-full bg-(--color-accent)/60";
  return (
    <section className="relative isolate overflow-hidden bg-[linear-gradient(180deg,#ffffff_0%,#f7f1eb_100%)] py-14 sm:py-18 lg:py-24">
      ...
      <Container className="grid items-center gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:gap-16">
        <div className="max-w-2xl">
          <p className="mb-6 inline-flex items-center gap-2 rounded-full border border-(--color-primary) bg-white px-4 py-2 text-sm font-semibold text-(--color-primary) shadow-sm">
            <span className="size-2 rounded-full bg-(--color-accent)" />
            Fizjoterapia w Katowicach i Mysłowicach
          </p>
          <h1 className="font-(--font-display) text-[2.75rem] leading-[1.03] text-(--color-primary) sm:text-6xl lg:text-7xl">
            Bogacki Fizjoterapia - skuteczna{" "}
            <span className="relative whitespace-nowrap text-(--color-primary)">
              pomoc w bólu
              <span className={newLocal} />
            </span>
          </h1>
          <p className="mt-7 max-w-xl text-lg leading-8 text-(--color-text-muted) sm:text-xl">
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {trustItems.map((item) => (
              <div
                key={item}
                className="flex items-center gap-3 rounded-2xl border border-(--color-border) bg-white px-4 py-3 text-sm font-semibold text-(--color-primary) shadow-sm"
              >
                <CheckCircle2
                  size={17}
        ...
        <div className="relative">
          <div className="absolute -inset-4 -z-10 rounded-[2.5rem] bg-(--color-primary)/12 blur-2xl" />
          <PlaceholderImage
            src="/gabinet.webp"
